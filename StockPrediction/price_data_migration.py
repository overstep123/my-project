...
def get_price(today,bar,count):
    ...
    stock_df = pd.read_sql_query("select code,name from stock_list;",engine)
    ...
    for i in stock_list:
        ...
        if(df.empty):# 异常情况数据为空
            ...
            df[['date','open','high','close','low','code','stockname','volume']].to_sql('stockprediction_stock_price',engine,if_exists='append',index=False)
